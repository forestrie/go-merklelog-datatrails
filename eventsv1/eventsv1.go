...
func VerifiableEventsV1EventsFromData(data []byte, logTenant string) ([]appentry.AppEntry, error) {

	// Accept either the list events response format or a single event. Peak
	// into the json data to pick which.
	eventsJson, err := datatrails.EventListFromJson(data)
	if err != nil {
		return nil, err
	}
...
func NewEventsV1AppEntries(eventsJson []byte, logTenant string) ([]appentry.AppEntry, error) {
...
func NewEventsV1AppEntry(eventJson []byte, logTenant string) (*appentry.AppEntry, error) {
